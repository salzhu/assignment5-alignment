...
def train_grpo(model_name, 
    ...
    wandb.init(
        project="a5-grpo",
        name=f"grpo_lr{learning_rate}",  # Set your run name here
    )
        ...
        for epoch in range(epochs_per_rollout_batch):
            ...
            for idx, (input, labels, mask, old_log_probs, advantage) in enumerate(dataloader):
                ...
                wandb.log({
                    ...
                    "train_step": epoch * len(dataset) + idx + 1
                })

                if (epoch * len(dataset) + idx + 1) % gradient_accumulation_steps == 0:
                ...
                if (epoch * len(dataset) + idx + 1) % eval_steps == 0: 
                    load_policy_into_vllm_instance(policy, llm)
